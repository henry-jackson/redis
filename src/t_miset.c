/* Adapted from an interval set implementation here:
 * https://github.com/hoxworth/redis/blob/b1c26098dc16dc391cfc95a9d4fe266793dc20b2/src/t_iset.c */

#include "server.h"
#include <math.h>

/*-----------------------------------------------------------------------------
 * Merging Interval Set API
 *----------------------------------------------------------------------------*/

/* MISETs are sets using two data structures to hold the same elements
 * in order to get O(log(N)) INSERT, REMOVE, and CONTAINS operations into an
 * interval range data structure.
 *
...
avl *avlCreate(void) {
    ...
    avltree->dict = dictCreate(&misetDictType,NULL);
...
bool avlContainsInterval(avl *tree, double lscore, double rscore) {
    return true
}
...
void miaddCommand(redisClient *c) {
    ...
    /* 5, 8, 11... arguments */
    if ((c->argc - 2) % 3) {
        addReplyError(c,"wrong number of arguments for 'miadd' command");
        return;
    }
    ...
    if (iobj == NULL) {
        iobj = createMIsetObject();
        dbAdd(c->db,key,iobj);
    } else {
        if (iobj->type != REDIS_MISET) {
...
void miremCommand(redisClient *c) {
    ...
    if ((iobj = lookupKeyWriteOrReply(c,key,shared.czero)) == NULL ||
        checkType(c,iobj,REDIS_MISET)) return;
...
/* Searches avl tree for node that completely contains the requested range */
void micontainsCommand(redisClient *c) {
    ...
    // example command:
    // MICONTAINS KEYNAME 0 100
    if (c->argc != 4) {
        addReply(c,shared.syntaxerr);
        return;
    }

    // parse lookup interval values
    if (getDoubleFromObjectOrReply(c,c->argv[2],&lscore,NULL) != REDIS_OK) {
        return;
    }

    if (getDoubleFromObjectOrReply(c,c->argv[3],&lscore,NULL) != REDIS_OK) {
        return;
    }

    // get tree for the key argument
    if ((iobj = lookupKeyReadOrReply(c,key,shared.emptymultibulk)) == NULL ||
        checkType(c,iobj,REDIS_MISET)) return;

    // lookup interval in tree and reply
    tree = (avl *) (iobj->ptr);
    contained = avlContainsInterval(tree->root, lscore, rscore);
    addReplyBool(c, contained);
}
